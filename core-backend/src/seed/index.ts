...
// Default = clean production-style seed: real geo master data (districts, roads,
// bridges, corridor routes) + ONLY the two login accounts (admin + transporter).
// All mock business data (vehicles, drivers, deliveries, field reports, alerts)
// stays OUT of the database — real records are added through the dashboards.
// Set SEED_FULL_DEMO=1 to also load the full demo dataset for presentations.
const FULL_DEMO = process.env.SEED_FULL_DEMO === '1';

export async function runSeed() {
  console.log('🌱 Starting database seed for Raahi Platform...');
  if (FULL_DEMO) console.log('🎪 SEED_FULL_DEMO=1 → loading full demo dataset too.');
  else console.log('🧹 CLEAN SEED → wiping all mock business data. Only geo master + 2 login users.');
  ...
  // 1. Seed Users — ONLY the two dashboard login accounts in clean mode
  const passwordHash = await bcrypt.hash('admin123', 12);
  const transporterHash = await bcrypt.hash('transporter123', 12);
  const driverHash = await bcrypt.hash('driver123', 12);

  // Real reference fleet: exactly ONE driver login (for the Driver Web App),
  // linked to one driver record + one assigned vehicle + one planned trip so
  // the REAL GPS pipeline has an honest end-to-end assignment to exercise.
  ...
  if (FULL_DEMO) {
    const officialHash = await bcrypt.hash('official123', 12);
    const userHash = await bcrypt.hash('user123', 12);
    await User.bulkCreate([
      {
        id: 'usr_official_002', name: 'Debashis Hazarika', email: 'd.hazarika@nhidcl.gov.in',
        password_hash: officialHash, role: 'district_officer', district_id: 'kamrup',
        agency: 'NHIDCL Regional Office Guwahati', phone: '+91 9876500002',
      },
      {
        id: 'usr_user_005', name: 'Arun Sharma', email: 'arun.sharma@northeastlogistics.in',
        password_hash: userHash, role: 'viewer', agency: 'Assam Essential Supplies Corporation', phone: '+91 9876500005',
      },
    ]);
  }
  console.log('✅ Users seeded (incl. driver: driver@raahi.gov.in / driver123).');

  // 1b. Reference driver + vehicle + trip (real GPS integration fixture)
  await Vehicle.bulkCreate([
    {
      id: 'AS-01-AB-1234', model: 'Tata 407 LPT', transporter_id: 'transporter_01', type: 'Light Commercial Vehicle',
      capacity_kg: 3500, status: 'idle', current_lat: 26.1445, current_lng: 91.7362, speed: 0,
      fuel_percent: 78, current_route: null, assigned_driver_id: 'DRV-01', tracking_active: false, live_status: 'OFFLINE',
    },
  ]);
  await Driver.bulkCreate([
    {
      id: 'DRV-01', name: 'Rakesh Das', phone: '9876543210', license_number: 'AS-0120190045123',
      license_expiry: new Date('2028-12-31'), vehicle_id: 'AS-01-AB-1234', transporter_id: 'transporter_01',
      user_id: 'usr_driver_004', status: 'active', rating: 4.9,
    },
  ]);
  console.log('✅ Reference driver + vehicle (AS-01-AB-1234) seeded.');
  ...
  await District.bulkCreate(districtsData as any);
  ...
  await Road.bulkCreate(roadsData as any);
  ...
  await Bridge.bulkCreate(bridgesData as any);
  ...
  await Route.bulkCreate(routesData as any);
  ...
  // Trip references route R-02 above, so it is created after the routes.
  await Trip.bulkCreate([
    {
      id: 'TRP-1001', vehicle_id: 'AS-01-AB-1234', driver_id: 'DRV-01', route_id: 'R-02', transporter_id: 'transporter_01',
      origin: 'Guwahati', destination: 'Shillong', status: 'planned', progress_percent: 0,
      started_at: new Date(), eta: new Date(Date.now() + 2.5 * 3600 * 1000),
    },
  ]);
  console.log('✅ Reference trip (TRP-1001 Guwahati→Shillong) seeded.');

  // 6. Seed Vehicles & Drivers (DEMO ONLY in FULL_DEMO mode)
  if (FULL_DEMO) {
  await Vehicle.bulkCreate([
  ...
  console.log(FULL_DEMO ? '✅ Vehicles and Drivers seeded.' : '✅ Vehicles & Drivers table is EMPTY (add real fleet from dashboard).');

  // 7. Seed Deliveries / Consignments (DEMO ONLY in FULL_DEMO mode)
  if (FULL_DEMO) {
  ...
  console.log(FULL_DEMO ? '✅ Deliveries seeded.' : '✅ Deliveries table is EMPTY (real consignments added from dashboard).');

  // 8. MongoDB Field Reports — always wiped; only demo-seeded in FULL_DEMO mode
  await FieldReport.deleteMany({});
  if (FULL_DEMO) {
  await FieldReport.insertMany([
    {
      id: 'FR-2025-0128',
      type: 'Road Damage',
      iconType: 'damage',
  ...
  console.log(FULL_DEMO ? '✅ MongoDB Field Reports seeded.' : '✅ MongoDB Field Reports cleared.');

  // 9. MongoDB Alerts — always wiped; only demo-seeded in FULL_DEMO mode
  ...
  console.log(FULL_DEMO ? '✅ MongoDB Alerts seeded.' : '✅ MongoDB Alerts cleared.');
