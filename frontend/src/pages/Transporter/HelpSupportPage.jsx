...
export default function HelpSupportPage() {
  ...
  // Tickets the user ACTUALLY raised in this session — real, not pre-seeded.
  ...
  const handleSelectResource = (resourceId) => {
    if (resourceId === 'status') {
      triggerToast('Platform status is shown live in your dashboard — alerts appear there in real time.');
    } else {
  ...
  const handleSubmitTicket = (ticketData) => {
    ...
    const now = new Date();
    const realTicket = {
      id: `SUP-${now.getFullYear()}-${String(now.getTime()).slice(-6)}`,
      subject: ticketData.subject,
      priority: ticketData.priority,
      category: ticketData.category,
      date: now.toLocaleString([], { day: '2-digit', month: 'short', year: 'numeric', hour: '2-digit', minute: '2-digit' }),
      status: 'Open',
    };
    setTickets((prev) => [realTicket, ...prev]);
    triggerToast(`Support ticket ${realTicket.id} created. Our team will respond shortly.`);
  };

  const handleViewTicket = (ticket) => {
    triggerToast(`Ticket ${ticket.id}: ${ticket.subject} (${ticket.status}).`);
  };

  const handleGuides = () => {
    triggerToast('Browse the help topics below for guides and answers.');
  };
            ...
            <div className="grid grid-cols-1 lg:grid-cols-12 gap-5 items-stretch">
              ...
              <div className="lg:col-span-5 xl:col-span-5 h-full">
                <HelpContactChannelsCard
                  ...
                  onStatus={() => handleSelectResource('status')}
                />
              </div>
            </div>
