...
export default function AlertsPage() {
    ...
    try {
      ...
      if (res?.success && Array.isArray(res.data)) {
        const mapped = res.data.map((a) => ({
          id: a.id,
          title: a.title || 'Route Alert',
          severity: a.severity || 'Medium',
          severityType: (a.severityClass || a.severity || 'medium').toLowerCase(),
          origin: a.location || districtName(a.districtId) || 'Assam corridor',
          destination: a.routeId ? `${a.routeId} corridor` : 'Fleet corridor',
          subtitle: a.message || a.type || 'Route disruption advisory',
          image: a.image || null,
          timestamp: a.createdAt ? new Date(a.createdAt).toLocaleString([], { day: 'numeric', month: 'short', hour: '2-digit', minute: '2-digit' }) : (a.time || ''),
  ...
  useEffect(() => {
    loadAlerts();
    const timer = setInterval(loadAlerts, 20000);
