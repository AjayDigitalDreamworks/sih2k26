...
export default function SettingsPage() {
  ...
  // Editable states — initialized from the REAL account record (server),
  // never from a hardcoded persona.
  ...
  // Load the real profile from /auth/me (hydrated with agency/phone from DB)
  ...
  const handleSaveCompany = async (newData) => {
    // Persist the fields that live on the account row (agency + phone).
    // Email is the login identity and is intentionally read-only.
    setProfileSaving(true);
    try {
      const payload = {
        agency: newData.companyName || null,
        phone: newData.businessPhone || null,
      };
      const res = await ApiClient.updateOwnProfile(payload);
      if (res?.success) {
        setCompanyData((prev) => ({ ...prev, companyName: newData.companyName, businessPhone: newData.businessPhone || '' }));
        setEditCompanyOpen(false);
        triggerToast('Company information updated successfully.');
        await loadProfile();
      } else {
        triggerToast(res?.message || 'Could not save company information.');
      }
    } catch (e) {
      console.error(e);
      triggerToast('Server error while saving company information.');
    } finally {
        ...
        <main className="flex-1 p-4 sm:p-5 lg:p-6 space-y-5 relative max-w-[1600px] w-full mx-auto">
          ...
          <div className="space-y-5">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-5 items-stretch">
              {profileLoading ? (
                <div className="bg-white rounded-2xl border border-slate-200/80 shadow-2xs p-6 text-sm text-slate-500 font-medium">
            ...
            <footer className="pt-2 pb-4 flex flex-col sm:flex-row items-center justify-between text-xs text-slate-400 font-medium gap-2">
              <span>© 2026 Raahi Transporter. All rights reserved.</span>
