  ...
  return (
    <div className="bg-white rounded-2xl border border-slate-200/80 shadow-2xs p-4 sm:p-5 flex flex-col justify-between h-full">
      ...
      <div className="flex-1 flex flex-col justify-between space-y-3.5 my-1">
        {alerts.length === 0 && (
          <div className="flex-1 min-h-[96px] p-4 rounded-2xl border border-dashed border-emerald-200 bg-emerald-50/40 flex items-center justify-center">
            <span className="text-xs font-semibold text-emerald-700">✓ All clear — no active alerts right now</span>
          </div>
        )}
        {alerts.map((alert) => (
          <div
