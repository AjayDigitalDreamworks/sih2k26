...
// Real current-device description from the browser (no invented sessions).
function currentDevice() {
  try {
    ...
    const os = isMac ? 'macOS' : isWin ? 'Windows' : isAndroid ? 'Android' : isIOS ? 'iOS' : 'Desktop';
      ...
      <div className="fixed inset-0 z-[9999] overflow-y-auto bg-slate-900/60 backdrop-blur-xs flex items-center justify-center p-4">
        ...
        >
          ...
          <div className="p-5">
            ...
            {itemType === 'sessions' && (
              <div className="space-y-3">
                ...
                <div className="pt-3 border-t border-slate-100 flex items-center justify-between">
                  <span className="text-[10px] text-slate-400 leading-relaxed max-w-[55%]">
                    To revoke access from a lost device, contact your Raahi administrator.
                  </span>
                  <button
                    type="button"
                    onClick={onClose}
            ...
            {/* 4. Login Activity */}
            {itemType === 'activity' && (
              <div className="space-y-2.5">              <div className="space-y-2 text-xs">
                <div className="p-2.5 rounded-xl border border-slate-100 bg-slate-50 flex items-center justify-between">
                  <div>
                    <span className="font-bold text-slate-900 block">{device}</span>
                    <span className="text-[10px] text-slate-400">This browser</span>
                  </div>
                  <span className="text-[10px] font-bold text-emerald-600">Now</span>
                </div>

                <p className="pt-1 text-[10px] text-slate-400 leading-relaxed">
                  Historical login activity is not stored on this device — check server audit logs
