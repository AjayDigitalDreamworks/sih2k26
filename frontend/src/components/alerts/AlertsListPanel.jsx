  ...
  return (
    <div className="bg-white rounded-2xl border border-slate-200/80 shadow-2xs p-4 sm:p-5 flex flex-col justify-between h-full">
          ...
          {alerts.length === 0 ? (
            ...
            alerts.map((alert) => {
              ...
              return (
                ...
                >
                  {/* Left: Image Thumbnail (fallback = severity chip when no photo) */}
                  <div className="w-16 h-16 sm:w-18 sm:h-18 rounded-xl overflow-hidden flex-shrink-0 bg-slate-100 border border-slate-200/70 flex items-center justify-center">
                    {alert.image ? (
                      <img
                        src={alert.image}
                        alt={alert.title}
      ...
      <div className="pt-4 border-t border-slate-100 flex flex-col sm:flex-row items-center justify-between gap-3 text-xs text-slate-500 mt-4">
        <span>Showing {alerts.length > 0 ? 1 : 0} to {alerts.length} alerts</span>
